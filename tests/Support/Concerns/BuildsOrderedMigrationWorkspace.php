...
trait BuildsOrderedMigrationWorkspace
{
    ...
    private function orderedMigrationSourceFiles(): array
    {
        ...
        // z_-prefixed migrations add columns to tables created by other packages,
        // so they must run after all non-z_ package migrations regardless of registration order.
        $basePackageMigrations = array_filter($packageMigrations, static fn (string $path): bool => ! str_starts_with(basename($path), 'z_'));
        $latePackageMigrations = array_filter($packageMigrations, static fn (string $path): bool => str_starts_with(basename($path), 'z_'));

        $orderedMigrations = array_merge($testMigrations, $coreMigrations, array_values($basePackageMigrations), array_values($latePackageMigrations));
