# SEO Tools Expansion Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship a complete Capell SEO Tools expansion with editor previews, scoring, expanded audits, redirect opportunities, internal-link suggestions, schema templates, Search Console insights, publish gates, and AI content briefs.

**Architecture:** SEO Tools owns SEO reports, previews, schema template registration, Search Console clients, internal-link suggestions, and AI briefs through Actions and Data objects. Redirect functionality remains in `capell-app/redirects`; SEO Tools only integrates with it through public Actions/contracts and graceful installed-package checks. Workspaces delegates SEO publish severity to SEO Tools report data instead of duplicating SEO logic.
...
enum SeoIssueSeverityEnum: string implements HasLabel
{
    ...
    {
        return match ($this) {
            self::Critical => __('capell-seo-tools::generic.seo_severity_critical'),
            self::Warning => __('capell-seo-tools::generic.seo_severity_warning'),
            self::Notice => __('capell-seo-tools::generic.seo_severity_notice'),
            self::Passed => __('capell-seo-tools::generic.seo_severity_passed'),
        };
    }
...
Modify `SeoToolsServiceProvider::registerPageSchemaExtenders()` to tag:
...
- Does not write directly to `page_urls` from SEO Tools.
...
Register `WebPageSchemaTemplate` and `ArticleSchemaTemplate` in `SeoToolsServiceProvider`. The default templates should delegate to existing schema Actions where possible.
...
- [ ] **Step 6: Wire schema reports into page SEO reports and graph**

`BuildPageSeoReportAction` should include schema reports. `SchemaGraphAction` should ask the registry for matching templates and merge valid template nodes into the graph without duplicating existing WebPage/Article nodes.
...
- [ ] **Step 4: Bind provider in SEO Tools**

In `SeoToolsServiceProvider`, bind `SeoPublishReportProvider::class` to `SeoPublishReportProviderAdapter::class`.

- [ ] **Step 5: Update Workspaces check**

`SeoMetaCheck` should:

- Use `app()->bound(SeoPublishReportProvider::class)` safely.
- Fall back to its current DB check when SEO Tools is unavailable.
...
Add an `ai_content_brief` prompt to `capell-seo-tools.php`. Require JSON output and include the report fields.
...
Update `SeoToolsBoundaryTest` so SEO Tools is allowed to use public Redirects contracts/actions only in redirect integration classes, and does not import Blog, Tags, Site Search, or Workspaces internals from core report classes.
...
- Redirect scope: redirects remain in `capell-app/redirects`; SEO Tools integrates through public APIs and installed-package checks.
- Type consistency: all planned classes use `Capell\SeoTools` except Redirects table updates and Workspaces publish check integration.
- Test strategy: starts with Action tests, then Filament/admin integration, then cross-package package tests.
- Risk: the editor panel depends on the exact Filament schema context available in page edit forms. If the form context cannot provide saved page/site/language reliably, implement the panel first as a page header action/modal and then move it inline once the context hook is confirmed.
