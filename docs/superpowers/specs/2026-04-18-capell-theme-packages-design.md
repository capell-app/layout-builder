...
Create three modern, full-featured theme packages for Capell CMS that serve as foundational starter templates for developers building client websites. Each theme demonstrates a distinct design philosophy (minimalist corporate, creative agency, SaaS) while sharing a unified admin settings interface. Themes work as standalone Blade-based templates and optionally integrate with Mosaic for visual layout editing.
...
- Showcase Capell's capabilities (multi-language, workspaces, widgets, caching) through well-designed examples
- Demonstrate best practices in Blade structure, Tailwind organization, and component design
- Enable seamless integration with Mosaic when installed, without requiring it
...
✅ Components work as Blade templates AND Mosaic widgets (when mosaic installed)
...
- Mosaic Widget: `HeroSectionWidget.php`
...
- Mosaic Widget: `FeaturesGridWidget.php`
...
- Mosaic Widget: `TeamGridWidget.php`
...
- Mosaic Widget: `CaseStudiesCarouselWidget.php`
...
- Mosaic Widget: `FooterWidget.php`
...
- Mosaic Widget: `PortfolioGalleryWidget.php`
...
- Mosaic Widget: `ProcessFlowWidget.php`
...
- Mosaic Widget: `ClientTestimonialsWidget.php`
...
- Mosaic Widget: `ServicesGridWidget.php`
...
- Mosaic Widget: `ContactCtaWidget.php`
...
- Mosaic Widget: `FeaturesShowcaseWidget.php`
...
- Mosaic Widget: `PricingTableWidget.php`
...
- Mosaic Widget: `IntegrationsGridWidget.php`
...
- Mosaic Widget: `UseCasesGridWidget.php`
...
- Mosaic Widget: `SocialProofWidget.php`
...
- Mosaic Widget: `FaqAccordionWidget.php`
...
- No Mosaic widget (not a content section, part of layout)
...
## 5. Mosaic Widget Integration

### 5.1 Widget Registration

When Mosaic is installed, each theme's ServiceProvider registers **only its unique widgets** (not shared across themes):
...
Each widget extends `Capell\Mosaic\Models\Widget` and implements editable properties:
...
Widgets serialize to `layouts.layout_items` table (Mosaic's layout item storage):
...
### 5.4 Blade Components vs Mosaic Widgets

**Static Blade usage (works without Mosaic):**
```blade
...
**Mosaic widget usage (with Mosaic installed):**
...
Each theme seeds **2–3 example layouts** that use its widgets. When editor installs theme, layouts are available in Mosaic:
...
- Seeds theme record
- Registers Mosaic widgets (if installed)
- Publishes views/CSS
...
### 8.1 Layout Composability (Mosaic + Static)
...
<x-theme::services-grid :items="$services" />  <!-- Add agency's services widget -->
...
>
    ...
    <x-slot name="cta-button">
        <a href="/demo" class="btn-custom">Book a Demo</a>
    </x-slot>
</x-corporate::hero-section>
```
...
export default {
  content: [
    'resources/views/vendor/capell-themes/corporate/**/*.blade.php',
  ],
  theme: {
    extend: {
      colors: {
...
**Actions:**
- `InstallThemeActionTest` — verifies theme record creation, Mosaic registration, view publishing, layout seeding

**Blade Rendering:**
- `ComponentRenderingTest` — render each Blade component with sample data, assert expected HTML structure

**Widget Integration:**
- `WidgetRegistrationTest` — if Mosaic installed, verify all theme-specific widgets registered
...
- Mosaic widget schema (if applicable)
- Example usage
...
2. **Phase 2: Corporate Theme (Complete)**
   ...
   - Create 7 Mosaic widget classes (`src/Widgets/`)
   ...
   - Implement ServiceProvider: register widgets (if Mosaic installed), register with CapellCore
...
3. **Phase 3: Agency Theme (Complete)**
   ...
   - Create 9 Mosaic widgets with agency-specific schema
...
4. **Phase 4: SaaS Theme (Complete)**
   ...
   - Create 9 Mosaic widgets with SaaS-specific schema
...
6. **Phase 6: Mosaic Integration Testing**
   ...
   - Test layout preview: verify layouts render identically in frontend and in Mosaic preview
...
| Mosaic widget registration fails silently if Mosaic not installed | Use `class_exists()` check + test for both scenarios (with/without Mosaic) |
| Components too opinionated, hard to customize | Provide extensive documentation + slot-based architecture for flexibility |
| Performance issues with 8 components on one page | Lazy load via Blade conditional or use Mosaic visibility toggles |
...
- ✅ Mosaic integration seamless (widget editing matches Blade rendering pixel-for-pixel)
