...
`capell-app/seo-suite` already provides page SEO fields, social metadata, robots/canonical handling, XML sitemaps, JSON-LD `@graph`, `/llms.txt`, broken-link records, translation coverage dashboard-dashboard_reports, AI-assisted title/meta generation, sitemap admin tools, and render hooks.

`capell-app/redirects` already provides the redirect manager. It stores redirects in Core's `page_urls` table, supports manual 301/302 redirects, automatic redirects when page URLs change, CSV import/export, duplicate/self/loop/chain validation, frontend resolution, hit counts, and admin CRUD. SEO Suite must reuse and enhance this package instead of creating a second redirect system.

`capell-app/search` provides search logging and drivers. SEO Suite can use the same content/indexing vocabulary for internal-link suggestions, but SEO Suite must not reach into Search internals unless the dependency is explicit and justified.
...
- `packages/seo-suite/src/Enums/SeoIssueSeverityEnum.php`
...
Do not rebuild redirects inside SEO Suite. Improve `capell-app/redirects` and connect SEO Suite to it.

Improvements:

- Add a "Create redirect" action from SEO Suite broken-link and 404 tables when Redirects is installed.
...
- Cross-package coupling should use contracts, optional class checks, events, or admin action registration. SEO Suite must not duplicate redirect validation rules.

If Redirects is not installed, SEO Suite should show broken-link/404 information without redirect actions.
...
- `packages/seo-suite/src/Data/InternalLinkSuggestionData.php`
...
- SEO Suite owns the schema template registry.
...
- Page type packages can provide defaults without SEO Suite importing their internals.
...
- Show top declining pages in the SEO Suite admin area.
- Keep insights summary intentionally small and action-oriented.

No ranking guarantees, no broad insights dashboard, and no replacement for a future insights package.
...
Expand the existing PublishingStudio `SeoMetaCheck` concept into configurable SEO publish gates.
...
- The PublishingStudio package should integrate through contracts/events or a small adapter, without moving SEO logic into PublishingStudio.
...
Reuse existing SEO Suite AI infrastructure:
...
SEO Suite may depend on Core, Admin, Frontend, and optionally declared companion packages. It must not import package internals without a declared dependency and a clear integration boundary.
...
Blog, Tags, Search, PublishingStudio, and future Events integrations should be optional extension points.
...
- Arch tests confirming SEO Suite does not import companion package internals outside approved integration namespaces.
