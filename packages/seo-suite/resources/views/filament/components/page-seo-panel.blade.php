    ...
    @else
        ...
        <section class="border-t border-gray-200 pt-4 dark:border-gray-700">
            <div class="text-sm font-medium text-gray-950 dark:text-white">
                {{ __('capell-seo-suite::generic.seo_panel_links') }}
            </div>
        ...
        <section class="border-t border-gray-200 pt-4 dark:border-gray-700">
            <div class="text-sm font-medium text-gray-950 dark:text-white">
                {{ __('capell-seo-suite::generic.seo_panel_schema') }}
            </div>
            @if ($schemaIssues === [] && $report->schemaDashboardReports === [])
                <div class="mt-2 text-sm text-gray-600 dark:text-gray-300">
                    {{ __('capell-seo-suite::generic.seo_panel_section_clear') }}
                </div>
            @else
                ...
                >
                    ...
                    @foreach ($report->schemaDashboardReports as $schemaReport)
                        <li>
                            <span class="font-medium">
        ...
        <section class="border-t border-gray-200 pt-4 dark:border-gray-700">
            <div class="text-sm font-medium text-gray-950 dark:text-white">
                {{ __('capell-seo-suite::generic.seo_panel_redirects') }}
                ({{ count($redirectOpportunities) }})
            </div>
            @if ($redirectOpportunities === [])
                <div class="mt-2 text-sm text-gray-600 dark:text-gray-300">
                    {{ __('capell-seo-suite::generic.seo_panel_no_redirects') }}
                </div>
            @else
                <ul
                    class="mt-2 space-y-2 text-sm text-gray-700 dark:text-gray-300"
                >
                    @foreach ($redirectOpportunities as $opportunity)
                        <li>
                            ...
                            <span class="text-gray-500 dark:text-gray-400">
                                {{ __('capell-seo-suite::generic.seo_panel_redirect_hits', ['count' => $opportunity->hits]) }}
                            </span>
        ...
        <section class="border-t border-gray-200 pt-4 dark:border-gray-700">
            <div class="text-sm font-medium text-gray-950 dark:text-white">
                {{ __('capell-seo-suite::generic.seo_panel_search_console') }}
            </div>
        ...
        <section class="border-t border-gray-200 pt-4 dark:border-gray-700">
            ...
            >
                ...
                <div>
                    ...
                    <div class="mt-1">
                        {{ $report->robotsDirectives === [] ? __('capell-seo-suite::generic.seo_panel_default_robots') : implode(', ', $report->robotsDirectives) }}
                    </div>
                </div>
            </div>
        ...
        <section class="border-t border-gray-200 pt-4 dark:border-gray-700">
            <div class="text-sm font-medium text-gray-950 dark:text-white">
                {{ __('capell-seo-suite::generic.seo_panel_passed_checks_heading') }}
                ({{ count($passedCheckValues) }})
            </div>
            @if ($passedCheckValues === [])
                <div class="mt-2 text-sm text-gray-600 dark:text-gray-300">
                    {{ __('capell-seo-suite::generic.seo_panel_no_suggestions') }}
                </div>
            @else
                <ul
                ...
                >
                    @foreach ($passedCheckValues as $passedCheckValue)
                        ...
                        >
                            {{ __('capell-seo-suite::generic.seo_check_' . $passedCheckValue) }}
                        </li>
                    @endforeach
                </ul>
            @endif
