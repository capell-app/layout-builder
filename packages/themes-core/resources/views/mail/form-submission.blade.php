                    ...
                    <tr>
                        <td style="padding:24px 32px;">
                            <p style="margin:0 0 16px;font-size:14px;line-height:1.6;">
                                You received a new submission from the <strong>{{ $formName }}</strong> form
                                @if ($submittedFrom)
                                    on <strong>{{ $submittedFrom }}</strong>
                                @endif.
                            </p>
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
                                @foreach ($fields as $label => $value)
                                    <tr>
                                        <th align="left" style="padding:8px 12px;background:#f9fafb;border:1px solid #e5e7eb;font-size:12px;text-transform:uppercase;letter-spacing:0.05em;color:#6b7280;width:35%;">
                                            {{ $label }}
                                        </th>
