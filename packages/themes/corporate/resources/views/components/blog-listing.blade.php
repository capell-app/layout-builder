...
<section id="blog"
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <header class="mx-auto mb-12 max-w-2xl text-center">
            <h2 id="blog-title" class="text-3xl font-bold sm:text-4xl">{{ $title }}</h2>
            @if ($subtitle)
        ...
        <ul role="list" class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
            @forelse ($posts as $post)
                <li>
                    <article class="h-full overflow-hidden rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] shadow-sm transition hover:shadow-md">
                        @if (! empty($post['image']))
                            <img src="{{ $post['image'] }}"
                                 alt=""
                                 loading="lazy"
                        ...
                        <div class="p-6">
                            ...
                            @if (! empty($post['excerpt']))
                                <p class="mt-3 text-sm text-[var(--color-fg-muted)]">{{ $post['excerpt'] }}</p>
                            @endif
