...
<section id="portfolio"
    ...
    <div class="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-10">
        <header class="mb-12 flex flex-wrap items-end justify-between gap-6">
            <div class="max-w-2xl">
                <h2 id="portfolio-title" class="agency-display text-4xl sm:text-5xl md:text-6xl">{{ $title }}</h2>
                @if ($subtitle)
            ...
            @if (! empty($filters))
                <div role="tablist" aria-label="Portfolio filters" class="flex flex-wrap gap-2">
        ...
        <ul role="list" class="grid grid-cols-1 gap-6 sm:grid-cols-6 lg:grid-cols-12">
            @forelse ($projects as $project)
                ...
                @endphp
                <li class="{{ $span }}" data-category="{{ $project['category'] ?? 'all' }}">
                    <a href="{{ $project['url'] ?? '#' }}"
                       class="group block overflow-hidden rounded-[var(--radius-lg)] bg-[var(--color-bg-muted)] transition hover:shadow-[var(--shadow-card)]">
                        ...
                        <div class="flex items-center justify-between gap-4 p-6">
                            <div>
                                ...
                                <h3 class="mt-1 text-xl font-semibold leading-snug text-[var(--color-fg)]">
                                    {{ $project['title'] ?? '' }}
                                </h3>
                            </div>
