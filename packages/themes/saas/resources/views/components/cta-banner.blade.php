...
<section id="cta"
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="relative overflow-hidden rounded-2xl px-6 py-14 shadow-[var(--shadow-elevated)] sm:px-12 sm:py-20"
             style="{{ $bg }}">
            <div class="relative z-10 mx-auto max-w-2xl text-center">
                <h2 id="cta-banner-title"
                    class="text-3xl font-bold tracking-tight sm:text-4xl {{ $fg }}">
                    {{ $slot->isEmpty() ? $title : $slot }}
                </h2>
                @if ($subtitle)
                    <p class="mt-4 text-lg {{ $subtleFg }}">{{ $subtitle }}</p>
                @endif
                ...
                <div class="mt-8 flex flex-wrap items-center justify-center gap-3">
                    ...
                    @if ($secondaryCtaLabel && $secondaryCtaUrl)
                        <a href="{{ $secondaryCtaUrl }}"
                           class="inline-flex items-center rounded-lg border border-white/30 px-6 py-3 font-semibold {{ $fg }} transition hover:bg-white/10">
                            {{ $secondaryCtaLabel }}
                        </a>
                    @endif
                </div>
            ...
            @if ($variant === 'gradient')
                <div aria-hidden="true"
                     ...
                     style="background: radial-gradient(circle at 20% 20%, rgba(255,255,255,0.15), transparent 40%), radial-gradient(circle at 80% 80%, rgba(16,185,129,0.18), transparent 50%);"></div>
            @endif
        </div>
    </div>
</section>
