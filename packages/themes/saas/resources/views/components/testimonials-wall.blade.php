...
<section id="testimonials"
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        @if (empty($testimonials))
            <p class="text-center text-[var(--color-fg-muted)]">No testimonials configured.</p>
        @else
            <div class="{{ $gridCols }} gap-6 [column-fill:_balance]">
                @foreach ($testimonials as $t)
                    <figure class="mb-6 break-inside-avoid rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-muted)] p-6 shadow-[var(--shadow-card)]">
                        @if (! empty($t['rating']))
                            <div class="mb-3 flex items-center gap-0.5 text-[var(--color-accent)]" aria-label="Rating: {{ $t['rating'] }} out of 5">
                        ...
                        <blockquote class="text-[var(--color-fg)]">
                            <p class="text-base leading-relaxed">&ldquo;{{ $t['quote'] ?? '' }}&rdquo;</p>
                        </blockquote>
