...
class CapellUrlGenerator extends BaseUrlGenerator
{
    ...
    private function maybePrefixWithSiteBaseUrl(string $url): string
    {
        ...
        $useSiteDomain = config('capell-foundation-theme.use_site_domain_for_media', false);
        if ($useSiteDomain === false) {
            return $url;
        }

        $overrideBase = config('capell-foundation-theme.local_storage_url', '');
        $siteBase = config('capell-foundation-theme.site_base_url', '');
