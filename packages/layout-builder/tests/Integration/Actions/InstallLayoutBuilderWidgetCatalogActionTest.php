...
it('installs the layout-builder widget catalog with translations and expected metadata', function (): void {
    $english = Language::factory()->english()->create();
    $french = Language::factory()->create(['code' => 'fr', 'name' => 'French']);

    InstallLayoutBuilderWidgetCatalogAction::run(collect([$english, $french]), extraWidgets: true);
    ...
    expect($childrenWidget->translations()->count())->toBe(2)
        ...
        ->and($navigationTabsWidget->meta['view_file'])->toBe('capell-layout-builder::components.widget.navigation.tabs');
...
it('preserves customized widget meta on rerun while backfilling missing catalog keys', function (): void {
    ...
    expect(Widget::query()->where('key', 'gallery')->firstOrFail()->meta['margin'])
        ...
        ->toBe('capell-layout-builder::components.widget.navigation.tabs');
});
