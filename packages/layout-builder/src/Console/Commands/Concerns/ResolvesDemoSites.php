...
namespace Capell\LayoutBuilder\Console\Commands\Concerns;
...
trait ResolvesDemoSites
{
    ...
    private function getDemoSites(): array
    {
        ...
        $demoSitesOptions = collect([config('app.name')])
            ->merge($databaseSites)
            ->merge(collect(config('capell-demo-kit.pages', []))->map(
                static fn (array $demoSite): string => (string) ($demoSite['name']['en'] ?? ''),
            ))
            ->filter(static fn (string $site): bool => $site !== '')
            ->unique()
            ->mapWithKeys(static fn (string $site): array => [$site => $site])
            ->all();
