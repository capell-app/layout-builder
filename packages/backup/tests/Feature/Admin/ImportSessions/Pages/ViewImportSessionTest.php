...
use Capell\Backup\Filament\Resources\ImportSessions\Pages\ViewImportSession;
...
beforeEach(function (): void {
    ...
    $adminUser->givePermissionTo([
        ...
        InstallBackupPermissionsAction::PERMISSION_IMPORT_SESSION_VIEW,
        InstallBackupPermissionsAction::PERMISSION_IMPORT_SESSION_CANCEL,
        InstallBackupPermissionsAction::PERMISSION_IMPORT_SESSION_RETRY,
    ]);
...
function makeImportSession(array $overrides = []): ImportSession
{
    return ImportSession::query()->create(array_merge([
        ...
        'source_package_path' => 'exchanger/imports/package-' . Str::random(6) . '.zip',
    ], $overrides));
}
...
it('uses the installed import-session view permission for global admin resource access', function (): void {
    ...
    $user->givePermissionTo(InstallBackupPermissionsAction::PERMISSION_IMPORT_SESSION_VIEW);
...
it('denies import session resource access to non-global users with the view permission', function (): void {
    $user = test()->createUserWithPermission(InstallBackupPermissionsAction::PERMISSION_IMPORT_SESSION_VIEW);
    test()->actingAs($user);
