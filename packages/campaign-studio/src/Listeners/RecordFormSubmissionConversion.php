...
namespace Capell\CampaignStudio\Listeners;

use Capell\CampaignStudio\Actions\BuildConversionAttributionAction;
use Capell\CampaignStudio\Actions\RecordFormSubmissionConversionAction;
use Capell\CampaignStudio\Actions\ResolveCampaignLandingPageFromUrlAction;
...
final class RecordFormSubmissionConversion
{
    public function handle(object $event): void
    {
        ...
        $visit = $this->modelProperty($event, 'visit') ?? $this->modelProperty($event, 'insightsVisit');
        $insightsEvent = $this->modelProperty($event, 'event') ?? $this->modelProperty($event, 'insightsEvent');
        ...
        $attribution = $this->attribution($visit, $insightsEvent, $metadata, $landingPage);
    ...
    private function campaignTablesExist(): bool
    {
        ...
        $conversionsTableName = config('capell-campaign-studio.tables.conversions', 'campaign_conversions');
    ...
    private function attribution(
        ...
        $attribution = BuildConversionAttributionAction::run($visit, $insightsEvent);
