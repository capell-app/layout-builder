...
use Capell\PasswordSecurity\Actions\MarkUserForPasswordChangeAction;
...
class PasswordSecurityUserTableExtender implements UserTableExtender
{
    public function columns(): array
    {
        if (! $this->hasPasswordSecurityColumns()) {
            return [];
        }

        return [
            IconColumn::make('must_change_password')
                ->label(__('capell-password-security::users.must_change_password'))
                ->boolean()
                ->toggleable(),
            TextColumn::make('password_changed_at')
                ->label(__('capell-password-security::users.password_changed_at'))
                ->dateTime()
                ->sortable()
    ...
    public function filters(): array
    {
        ...
        return [
            TernaryFilter::make('must_change_password')
                ->label(__('capell-password-security::users.must_change_password')),
            Filter::make('password_security_expired')
                ->label(__('capell-password-security::users.password_expired'))
                ->query(fn (Builder $query): Builder => $this->expiredPasswordQuery($query)),
            Filter::make('password_security_missing_password_changed_at')
                ->label(__('capell-password-security::users.password_never_changed'))
    ...
    public function recordActions(): array
    {
        ...
        return [
            Action::make('require_password_change')
                ->label(__('capell-password-security::users.require_password_change'))
    ...
    private function hasPasswordSecurityColumns(): bool
    {
    ...
    private function canRequirePasswordChange(): bool
    {
        ...
        return $this->hasPasswordSecurityColumns() && $settings->forceChangeEnabled;
    }
