...
namespace Capell\PublishingStudio\Activity;
...
class WorkspaceActivityFeed
{
    ...
    public function recent(int $limit = 20): Collection
    {
        ...
        $publishingStudioById = Workspace::query()
            ->whereIn('id', $workspaceIds)
            ->get()
            ->keyBy('id');

        return $rows->map(function (Activity $row) use ($publishingStudioById): WorkspaceActivityEntry {
            $workspace = $publishingStudioById->get($row->subject_id);
