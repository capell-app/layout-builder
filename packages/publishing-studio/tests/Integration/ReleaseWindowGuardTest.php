...
it('dashboard-dashboard_reports open when the feature is disabled', function (): void {
    config()->set('capell.publishing-studio.release_windows.enabled', false);

    expect((new ReleaseWindowGuard)->isOpen())->toBeTrue();
});

it('dashboard-dashboard_reports open inside a configured day-and-time window', function (): void {
...
it('respects the configured timezone', function (): void {
    config()->set('capell.publishing-studio.release_windows.enabled', true);
    config()->set('capell.publishing-studio.release_windows.timezone', 'Europe/London');
    config()->set('capell.publishing-studio.release_windows.windows', [
