...
namespace Capell\Migrator\Listeners;
 ...
 * `migrator.notifications.recipients`.
 */
class SendImportSessionNotifications
{
    ...
    {
        if (! config('migrator.notifications.enabled', true)) {
            return;
        }
    ...
    {
        $roles = config(
            'migrator.notifications.recipients.' . $outcome,
            [],
        );
