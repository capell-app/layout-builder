...
The Media Assistant package adds optional AI-backed image actions to Capell's existing media resource. It does not replace the media backend, crop system, or localized metadata model.

Status: `Optional` · Surface: `Admin` · Depends on: `capell-app/admin`, `capell-app/core`

![Media Assistant action surface in the media library](images/screenshots/media-assistant-doctor-image.png)
...
- A `Capell\MediaAssistant\Contracts\ImageDoctor` contract that an assistant package can bind to the real image-editing implementation.
...
Assistant-backed packages should bind the contract in their service provider:

```php
use Capell\MediaAssistant\Contracts\ImageDoctor;

$this->app->bind(ImageDoctor::class, AssistantImageDoctor::class);
```
