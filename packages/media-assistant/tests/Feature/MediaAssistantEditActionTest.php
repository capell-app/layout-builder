...
use Capell\MediaAssistant\Tests\Fixtures\RecordingImageDoctor;
...
beforeEach(function (): void {
    test()->actingAs(createMediaAssistantGlobalUser());
...
function createMediaAssistantGlobalUser(): Authenticatable
{
    ...
    $user->forceFill([
        'name' => 'Media Assistant Admin',
        'email' => fake()->unique()->safeEmail(),
        'password' => bcrypt('password'),
    ])->save();

    Relation::morphMap(['test-media-assistant-admin-user' => $user::class], merge: true);

    return $user;
}

function createMediaAssistantImage(): CapellMedia
{
    $page = Page::factory()->create();

    /** @var CapellMedia $media */
    $media = $page
        ->addMedia(UploadedFile::fake()->image('assistant.jpg', 32, 32))
...
it('registers a media edit action extender when enabled', function (): void {
    expect(collect(app()->tagged(MediaEditActionExtender::TAG))
        ->contains(fn (object $extender): bool => $extender instanceof MediaAssistantEditActionExtender))
        ->toBeTrue();
});

it('keeps the doctor action hidden until an assistant-backed image doctor is bound', function (): void {
    expect(resolve(ImageDoctor::class))->toBeInstanceOf(NullImageDoctor::class);

    Livewire::test(EditMedia::class, [
        'record' => createMediaAssistantImage()->getRouteKey(),
    ])
        ->assertSuccessful()
        ->assertActionHidden('doctor-image');
});

it('passes image doctor requests to the configured assistant implementation', function (): void {
    $doctor = new RecordingImageDoctor;
    app()->instance(ImageDoctor::class, $doctor);

    $media = createMediaAssistantImage();
