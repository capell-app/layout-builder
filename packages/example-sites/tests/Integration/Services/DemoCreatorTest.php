...
use Capell\ExampleSites\Support\Creator\DemoResourceResolver;
...
it('rejects demo zip entries that escape the extraction directory', function (): void {
    ...
    $zipPath = tempnam(sys_get_temp_dir(), 'capell-example-sites-') . '.zip';
    ...
    config()->set('capell-example-sites.archive.checksum', hash_file('sha256', $zipPath));
...
it('rejects demo zip downloads with a checksum mismatch', function (): void {
    ...
    config()->set('capell-example-sites.archive.checksum', str_repeat('0', 64));
...
it('rejects demo zip downloads larger than the configured maximum size', function (): void {
    ...
    config()->set('capell-example-sites.archive.max_bytes', 5);
...
{
    $demoDirectory = sys_get_temp_dir() . '/capell-example-sites-resources-' . uniqid();
