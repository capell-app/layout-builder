...
class BlocksTable implements TableConfigurator
{
        ...
        return $table
            ->modifyQueryUsing(
                fn (Builder $query): Builder => $query->with([
                    ...
                    ->select('blocks.*')
                    /** @phpstan-ignore-next-line Block exposes this local scope through Eloquent. */
                    ->withLayoutsCount(),
            )
            ->defaultSort('name')
            ->columns(self::getTableColumns())
            ->filters(self::getTableFilters())
            ->recordClasses(fn (Block $record): ?string => match (true) {
    ...
    protected static function getTableColumns(): array
    {
        return [
            ...
            TextColumn::make('translation.content')
                ...
                ->toggleable(isToggledHiddenByDefault: true)
                ->formatStateUsing(
                    fn (ListBlocks $livewire, TextColumn $column, Block $record): string => Str::limit(
                        $record->translation->title ?? '',
                        $column->getCharacterLimit(),
                        $column->getCharacterLimitEnd(),
                    ),
                )
                ->description(function (ListBlocks $livewire, TextColumn $column, Block $record): ?HtmlString {
            ...
            TextColumn::make('component')
                ...
                ->size('xs')
                ->color(FilamentColorEnum::LightGray->value)
                ->formatStateUsing(function (Block $record): ?HtmlString {
                    $components = [
            ...
            TextColumn::make('layouts_count')
                ...
                ->disabledClick()
                ->formatStateUsing(
                    function (Block $record, int $state): ?HtmlString {
                        if ($state === 0) {
                            return null;
                        }
