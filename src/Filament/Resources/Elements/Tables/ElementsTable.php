...
class ElementsTable implements TableConfigurator
{
        ...
        return $table
            ->modifyQueryUsing(
                fn (Builder $query): Builder => $query->with([
                    ...
                    ->select('elements.*')
                    /** @phpstan-ignore-next-line Element exposes this local scope through Eloquent. */
                    ->withLayoutsCount(),
            )
            ->defaultSort('name')
            ->columns(self::getTableColumns())
            ->filters(self::getTableFilters())
            ->recordClasses(fn (Element $record): ?string => match (true) {
    ...
    protected static function getTableColumns(): array
    {
        return [
            ...
            TextColumn::make('translation.content')
                ...
                ->toggleable(isToggledHiddenByDefault: true)
                ->formatStateUsing(
                    fn (ListElements $livewire, TextColumn $column, Element $record): string => Str::limit(
                        $record->translation->title ?? '',
                        $column->getCharacterLimit(),
                        $column->getCharacterLimitEnd(),
                    ),
                )
                ->description(function (ListElements $livewire, TextColumn $column, Element $record): ?HtmlString {
            ...
            TextColumn::make('component')
                ...
                ->toggleable(isToggledHiddenByDefault: true),
            TextColumn::make('element_assets_count')
                ->label(__('capell-admin::table.total_assets'))
                ->counts('elementAssets')
                ->sortable()
                ->alignCenter()
                ->numeric()
                ->toggleable(),
            TextColumn::make('layouts_count')
                ...
                ->disabledClick()
                ->formatStateUsing(
                    function (Element $record, int $state): ?HtmlString {
                        if ($state === 0) {
                            return null;
                        }
                        ...
                        return new HtmlString(
                            Blade::render(
                                ...
                                [
                                    'state' => $state,
                                    'url' => AdminSurfaceLookup::resource(ResourceEnum::Layout)::getUrl('index', ['filters[layout_element_id][value]' => $record->key]),
                                ],
                            ),
                        );
                    },
    ...
    protected static function getTableFilters(): array
    {
        return [
            SelectFilter::make('blueprint_id')
                ->label(__('capell-layout-builder::form.element_type'))
                ->relationship(
                    name: 'type',
                    titleAttribute: 'name',
                    modifyQueryUsing: fn (Builder $query): Builder => $query->where(
                        'type',
                        LayoutTypeEnum::Element,
                    ),
                ),
