...
/**
 * @method static array<int, array<string, mixed>> run(?string $widget = null, ?string $variant = null, ?string $theme = null, ?int $limit = null)
 */
final class BuildWidgetVisualRegressionManifestAction
{
    use AsFake;
    use AsObject;
     ...
     */
    public function handle(?string $widget = null, ?string $variant = null, ?string $theme = null, ?int $limit = null): array
    {
        $entries = [];

        foreach (resolve(BlockRegistry::class)->all() as $definition) {
            if ($widget !== null && $definition->key !== $widget) {
                continue;
            }

            foreach ($definition->variants as $widgetVariant) {
                if ($variant !== null && $widgetVariant->key->value() !== $variant) {
                    continue;
                }

                foreach ($this->scenarios() as $scenario => $fixture) {
                    foreach (['mobile', 'tablet', 'desktop'] as $viewport) {
                        $entries[] = $this->entry($definition, $widgetVariant, $viewport, $theme, $scenario, $fixture);
                    }
                }
            }
        }
